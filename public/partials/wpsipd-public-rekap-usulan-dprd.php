...
<form method="post" name="pilihan-anggota" action="<?php echo "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]"; ?>">
	...
	<select name="status">
		...
		<option value='8'>Status Lain</option>
	</select>
	...
	if (($_POST['anggota'] !== '') && ($_POST['status']) !== ''){ 
		...
		switch ($status) {
			...
			case '6':
				...
				break;
			case '7':
				$title = "Rekapitulasi Usulan Anggora DPRD ".$nama_anggota." dengan Status Diteruskan ke SKPD</br>PER ".$update_at;
				$qstatus = " AND status_usul_teks = 'Diteruskan ke Forum SKPD'";
				break;
			case '8':
				$title = "Rekapitulasi Usulan Anggora DPRD ".$nama_anggota." dengan Status kosong</br>PER ".$update_at;
...
if ($tampilrekap) {
	...
	select 
		...
		b.`jml` AS `jml`, 
		c.`jml` AS `jmlvalidsetwan`, 
		d.`jml` AS `jmlmusrenbang`, 
		e.`jml` AS `jmlforum`, 
	...
	(select `created_user` AS `created_user`,
		count(`id`) AS `jml` 
	from `data_usulan` 
	where ((`tahun_anggaran` = 2022) and (`active` = 1) and (`id_reses` IS NOT NULL)) 
	...
	(select `created_user` AS `created_user`,
		count(`id`) AS `jml` 
	from `data_usulan` 
	where ((`tahun_anggaran` = 2022) and (`active` = 1) and (`id_reses` IS NOT NULL) and (status_usul_teks!='') and (status_usul_teks='Diteruskan ke Musrenbang Kecamatan')) 
	...
	(select `created_user` AS `created_user`,
		count(`id`) AS `jml` 
	from `data_usulan` 
	where ((`tahun_anggaran` = 2022) and (`active` = 1) and (`id_reses` IS NOT NULL) and (status_usul_teks!='') and (`status_usul_teks` = 'Diteruskan ke Forum SKPD')) 
	...
	(select `created_user` AS `created_user`,
		count(`id`) AS `jml` 
	from `data_usulan` 
	where ((`tahun_anggaran` = 2022) and (`active` = 1) and (`id_reses` IS NOT NULL) and (status_usul_teks='Diteruskan ke Musrenbang Kecamatan||Ditolak')) 
...
<THEAD>
	<tr>
		...
		<th class=\"text_tengah kiri atas kanan bawah\" colspan=\"8\">JUMLAH USULAN</td>
		<tr>
							...
							<th class=\"text_tengah kiri atas kanan bawah\" rowspan=\"2\">STATUS LAIN</td>
							<tr>
...
foreach ($rekap as $baris => $baris_rekap) {
	...
	$total2 += $baris_rekap['jmlvalidsetwan'];
	$total3 += $baris_rekap['jmlditolaksetwan'];
	$jmlvalid = $baris_rekap['jml']-$baris_rekap['jmlnostatus']-$baris_rekap['jmlvalidsetwan']-$baris_rekap['jmlditolaksetwan'];
	$total4 += $jmlvalid;
	$total5 += $baris_rekap['jmlmusrenbang'];
	$total6 += $baris_rekap['jmlditolakkec'];
	$total7 += $baris_rekap['jmlforum'];
	...
	<tr>
		...
		<td class=\"text_tengah kiri atas kanan bawah\">".$baris_rekap['jmlvalidsetwan']."</td>
		...
		<td class=\"text_tengah kiri atas kanan bawah\">".$baris_rekap['jmlmusrenbang']."</td>
